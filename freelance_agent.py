    ...
    def login_upwork(self):
        ...
        try:
            ...
            if "login" in page.url or "challenge" in page.url:
                ...
                # Gunakan sistem bantuan manusia yang sudah kita tingkatkan waktunya
                self.browser.request_human_help("Upwork Login (CAPTCHA/2FA/OTP)")
                
                if "login" in page.url or "challenge" in page.url:
                    logging.error("Failed to bypass login wall after waiting.")
                    return False
        ...
        if response:
            try:
                ...
                if match:
                    ...
                    # Cari index array [ pertama dan ] terakhir
                ...
                logging.error(f"Failed to parse LLM filter response: {e}. Raw response: {response[:200]}")
